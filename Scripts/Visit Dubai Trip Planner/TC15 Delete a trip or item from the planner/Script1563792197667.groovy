...
for (int i = 1; i <= 2; i++) {
	...
	WebElement temp = driver.findElement(By.cssSelector(('.tp_change-trip-items > li:nth-child(' + i + ') > div')))
	    if (elementHasClass(temp, 'tp_change-trip-box-active') == false) {		
			...
			int dialogCount = driver.findElements(By.cssSelector('#htmlBody > div.ui-dialog.ui-widget.ui-widget-content.ui-corner-all.ui-front.tp_confirm-dialog.ui-dialog-buttons')).size() //total trips in trip planner
			int dialogBtnCount = driver.findElements(By.cssSelector('.ui-dialog-buttonset > button')).size() //total trips in trip planner

        //WebUI.click(getElement('css', '#tp_add-activities-close'))
			println (dialogCount)
			println(dialogBtnCount)
        if (dialogCount==1 && dialogBtnCount==2) {
            println('************ Test passed *********')
