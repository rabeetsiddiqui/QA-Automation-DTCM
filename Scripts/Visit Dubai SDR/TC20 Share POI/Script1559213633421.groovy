...
WebUI.click(getElement('css','#POIS_categoryDataRoot > li:nth-child(1) > div > a:nth-child(2)'))
String NewUrl = WebUI.getUrl();
println (NewUrl)

//Matching that is it redirect to the detail page or not
if(DefaultUrl != NewUrl)
{
	println ("Success")
}
else(DefaultUrl == NewUrl).call(
	{
		println ("Failed")
	})


//To share the POI through Facebook
WebUI.click(getElement('css', '#plhcontent_0_heroWrap > div > div.bott-social-icon > ul > li:nth-child(1) > a'))
Thread.sleep(2000);
ArrayList tabs = new ArrayList (driver.getWindowHandles());
driver.switchTo().window(tabs.get(1));

String NewUrlafterShare = WebUI.getUrl();
println (NewUrlafterShare)
if(NewUrl != NewUrlafterShare)
{
	println ("Success")
}
else(NewUrl == NewUrlafterShare).call(
	{
		println ("Failed")
	})
